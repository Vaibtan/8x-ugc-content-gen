...
type InterviewQuestion = Readonly<{
  id: string;
  prompt: string;
  helper: string;
}>;
...
const questions: ReadonlyArray<InterviewQuestion> = [
  ...
  {
    id: "buyer",
    prompt: "Who buys it, and when do they look for it?",
    helper: "Think role, trigger, and the job they need done.",
  },
  {
    id: "why",
    prompt: "Why did you choose this problem?",
    helper: "A concrete moment is more useful than a polished origin story.",
  },
  {
    id: "belief",
    prompt: "What do you believe that most people in your space miss?",
    helper: "Your useful contrarian view often becomes your strongest content.",
  },
  {
    id: "story",
    prompt: "Tell one customer story or hard-earned lesson.",
...
const initialAnswers = (): ReadonlyArray<Answer> =>
...
export function VoiceInterview({
  ...
  const [answers, setAnswers] = useState<ReadonlyArray<Answer>>(initialAnswers);
  ...
  const progress = Math.round((completedAnswers / questions.length) * 100);
  ...
  const transcribeBlob = async (blob: Blob, index: number) => {
    setStage("transcribing");
    setError(null);
    retry.current = () => void transcribeBlob(blob, index);
    ...
    if (result.ok) setAnswer(index, result.value);
  ...
  return (
    <section className="pb-8">
          ...
          {stage === "recording"
            ...
            : stage === "transcribing"
              ...
              : stage === "generating"
                ...
                : "Your answers stay editable until you generate."}
        </p>
      </div>
      ...
      <Button
        ...
        disabled={busy || completedAnswers < questions.length}
        onClick={generate}
      >
        {stage === "generating" ? (
          <LoaderCircle className="animate-spin" size={18} />
